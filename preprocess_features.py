...
if not data_dir.exists():
    data_dir = Path('D:','CNC_Audio','continuo_ivo')
...
feature_names = np.unique(['__'.join(col.split('__')[1:3]) for col in df_modified.columns if any(x in col for x in ['letra','animales']) and all(x not in col for x in ['list','data','query','error','text','nouns','verbs'])])
...
df_modified = df_modified.drop([col for col in df_modified.columns if any(x in col for x in ['list','data','query','error','text','nouns','verbs'])],axis=1)
